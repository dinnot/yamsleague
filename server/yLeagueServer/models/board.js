...
const board = function (data) {
    ...
    this.recordPut = function (column, cell) {
        ...
        this.data.current.player = (this.data.current.player + 1) % 4;
    ...
    this.getPlayerFinalColumnScore = function (column, player) {
        ...
        const op1Result = this.getPlayerColumnScore(column, (player - 1 + 4) % 4);
        const op2Result = this.getPlayerColumnScore(column, (player + 1) % 4);
        let points = 0;
        ...
        if (this.getScoreWithBonus(playerResult) > this.getScoreWithBonus(op2Result)) points += this.getPoints(playerResult, op2Result);
        else points -= this.getPoints(op2Result, playerResult);
    ...
    this.getTeamFinalScore = function (team) {
        if (team === 1) return this.getPlayerFinalScore(0) + this.getPlayerFinalScore(2);
        else return this.getPlayerFinalScore(1) + this.getPlayerFinalScore(3);
    };
